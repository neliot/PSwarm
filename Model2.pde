  ...
  void populate() {
    ...
    for(int i = 0; i < this._swarmSize; i++) {
      try {
        ...
        S.add(new Particle(this._nextParticleId++,_grid/2 - rand.nextInt(_grid),(float)_grid/2 - rand.nextInt(_grid),0.0,this._Cb,this._Rb,this._speed));      } catch (Exception e) {
        println(e);
        exit();
      }
    }
