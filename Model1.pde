  ...
  void populate() {
    ...
    for(int i = 0; i < this._swarmSize; i++) {
      try {
        ...
        S.add(new Particle(this._nextParticleId++,_grid/2 - rand.nextInt(_grid),(float)_grid/2 - rand.nextInt(_grid),0.0f,this._Cb,this._Rb,this._speed));
      } catch (Exception e) {
        println(e);
        exit();
      }
...
*/
    ...
    for(Particle p : S) {      
      ...
      if (_loggingP) {
        if (_logMin) {
          pData += plog._counter + "," + p.logString(_logMin) + "," + coh.x + "," + coh.y + "," + coh.mag() + "," + rep.x + "," + rep.y + "," + rep.mag() + "," + inter.x + "," + inter.y + "," +inter.mag() + "," + dir.x + "," + dir.y + "," + dir.mag() + "\n";
        } else {
